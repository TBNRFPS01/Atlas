...
from core.providers import GatewayProvider, LocalProvider
...
def install_execution_pipeline(router: Router, config: ConfigManager) -> None:
    """Configure the router-owned execution pipeline without replacing its boundary."""
...
def main() -> None:
    ...
    if config.get("planner_enabled", True) and config.get("autonomy_enabled", False):
        ...
        goal_service.start()
        print("Autonomy: âœ“ Background goal service enabled")
    ...
    if voice_enabled:
        try:
            ...
            voice_thread.start()
            print("Voice: âœ“ Voice assistant enabled")
    ...
    # Health monitoring: register lightweight checks for core components and
    # start the background loop. Failures are visible via /status.
    health_monitor = HealthMonitor(check_interval=60.0)
    health_monitor.register_check(
        "llm",
        lambda: not brain.ask("ping").lower().startswith("lm studio connection failed"),
    )
    health_monitor.register_check(
        "memory",
        lambda: memory.db is not None,
    )
    health_monitor.register_check(
        "tools",
        lambda: len(registry.list()) > 0,
    )
    health_monitor.start()
    router._health_monitor = health_monitor
