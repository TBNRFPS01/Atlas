"""HTML-matched Tkinter composer for ATLAS.

The HTML composer is a fixed bottom region with a centered column.  The
Tkinter implementation deliberately uses pack geometry for the vertical
stack so the composer contributes its real height to the root window.
This avoids the classic Tkinter ``place()`` collapse where a positioned
child does not affect its parent's requested size.
"""
...
class Composer:
    """Bottom-fixed visual composer matching the HTML design."""
    ...
    def __init__(
        ...
        *,
        ...
        self._busy = False
        self._column_width = CHAT_MAX_WIDTH

        # This frame is packed by the root at the bottom.  It must own its
        # natural height, unlike the previous place()-based implementation.
        self.frame = tk.Frame(parent, bg=BACKGROUND)
        self.frame.pack(side="bottom", fill="x")

        tk.Frame(self.frame, bg=BORDER, height=1).pack(side="top", fill="x")

        self.wrap = tk.Frame(self.frame, bg=BACKGROUND)
        self.wrap.pack(fill="x", padx=24, pady=(20, 10))

        # IMPORTANT: pack, not place. This makes the column's requested
        # height part of wrap's geometry and keeps the composer visible.
        self.column = tk.Frame(self.wrap, bg=BACKGROUND, width=self._column_width)
        self.column.pack(side="top", anchor="center")
        ...
        self.btn_attach = RoundedButton(
            self.box, "＋", command=self._pick_files,
            bg=PANEL_BG, fg=INK_SOFT, hover_bg=BACKGROUND,
            font=(FONT_FAMILY, 15, "normal"), radius=12,
            width=40, height=40, outer_bg=PANEL_BG,
        )
        ...
        self.btn_voice = RoundedButton(
            self.box, "🎙", command=self._on_voice,
            bg=PANEL_BG, fg=INK_SOFT, hover_bg=BACKGROUND,
            font=(FONT_EMOJI, 14, "normal"), radius=12,
            width=40, height=40, outer_bg=PANEL_BG,
        )
        self.btn_voice.pack(side="right", padx=(2, 6), pady=8)

        self.btn_send = RoundedButton(
            self.box, "↑", command=self.send,
            bg=ACCENT, fg=BACKGROUND, hover_bg=ACCENT_HOVER,
            font=(FONT_FAMILY, 15, "bold"), radius=12,
            width=40, height=40, outer_bg=PANEL_BG,
        )
        self.btn_send.pack(side="right", padx=(2, 8), pady=8)

        self.btn_stop = RoundedButton(
            self.box, "■", command=self._on_stop,
            bg=ACCENT, fg=BACKGROUND, hover_bg=ACCENT_HOVER,
            font=(FONT_FAMILY, 12, "normal"), radius=12,
            width=40, height=40, outer_bg=PANEL_BG,
        )
    ...
    def _on_wrap_configure(self, event: tk.Event) -> None:
        """Keep the composer centered and capped like HTML max-width: 900px."""
        width = min(CHAT_MAX_WIDTH, max(240, event.width))
        if width != self._column_width:
            self._column_width = width
            self.column.configure(width=width)
    ...
    def get_text(self) -> str:
        ...
        if not raw.strip() or raw.strip() == "Message ATLAS...":
            return ""
        return raw.strip()
    ...
    def _pick_files(self) -> None:
        paths = filedialog.askopenfilenames(
            title="Attach files", parent=self.parent.winfo_toplevel()
        )
        for path in paths:
